...
module.exports = function getSimpleManifest () {
  ...
  const containerElements = ['Container', 'Column', 'ColumnSet', 'TabSet', 'Tab', 'Collapsible', 'ActionSet', 'ImageSet']
  ...
  filteredSchema.forEach(elementType => {
    ...
    if (Object.prototype.hasOwnProperty.call(elementDefinition, 'properties')) {
...
function calculatePropertySummary (elementType, elementProperties, rawElementDefinition) {
  ...
  elementProperties.forEach(prop => {
    if (elementType === prop.name && Object.prototype.hasOwnProperty.call(rawElementDefinition, 'required')) {
      Object.keys(rawProps).forEach(property => {
        const name = property
        const type = rawProps[property].type
        let required
